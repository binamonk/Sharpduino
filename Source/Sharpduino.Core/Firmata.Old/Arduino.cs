...
namespace Sharpduino.Firmata.Old
{
	/**
 ...
 */
	public class Arduino
	{
		...
		private const int SET_DIGITAL_PIN_MODE  = 0xF5; // set a digital pin to INPUT/OUTPUT
		private const int END_SYSEX             = 0xF7; // end a MIDI SysEx message
		private const int REPORT_VERSION        = 0xF9; // report firmware version

		private const int EXTENDED_STRING       = 0x71; // String
		private const int EXTENDED_VERSION      = 0x79; // Firmware name/version

		private const int SYSTEM_RESET          = 0xFF; // reset from MIDI



		private readonly CommTransports.ICommTransport _serialPort;
		private int _delay;
						...
						else if (_waitForData > 0 && inputData < 128)
						{
							...
							if (_executeMultiByteCommand != 0 && _waitForData == 0)
							{
								...
								case ANALOG_MESSAGE:
									...
									break;
								case REPORT_VERSION:
									setVersion(_storedInputData[1], _storedInputData[0]);
									break;
								}
							}
						}
